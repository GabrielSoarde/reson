...
**Sobre "paralelo" (calibração honesta):** assumindo projeto de uma pessoa (GabrielSoarde/reson, distribuição pra amigos), "paralelo" na prática é **intercalado** — esse track compete por horas com os milestones, não soma a eles. A separação continua certa (desacopla o risco), mas não trate como banda grátis: é "quando sobrar fôlego entre features". Se houver mais de uma pessoa no projeto, aí sim é simultâneo de verdade.

**Risco ativo até o T5 aterrissar:** o roadmap chama a máquina local de "ponto único de falha". Enquanto o T5 (CI/CD) não existe, **todo o M1 ainda depende dessa máquina pra publicar** (o `publish-release.ps1` local). Funciona hoje (estamos na v1.0.3), mas o risco que o T5 resolve permanece ativo durante o M1 inteiro — vale ter backup da chave de assinatura/keystore e do ambiente de build até o CI cobrir isso.
...
- **D1 — Modo festa multi-celular.** **Escopo decidido no review: v1 é trigger-only.** Vários celulares pareiam e todos *disparam* sons no mesmo board; o nome/cor do participante e o feedback de "quem tocou" são a feature. Disparar é POST `/api/play` — **não** esbarra na edição concorrente. **Edição colaborativa** (vários editando o mesmo board ao vivo) fica fora do D1 e **depende do T8 (M4)** — se for desejada, é um incremento posterior, não parte do modo festa inicial. Esta dependência D1-colaborativo → T8 está declarada aqui de propósito, não pra ser descoberta na implementação.
  - **Dependência de percepção (não só técnica):** "modo festa" no imaginário do usuário sugere "todo mundo mexe no board junto", mas o v1 só deixa *disparar*. Pra não gerar reclamação de "cadê a edição junto", comunicar o escopo: na UI chamar de algo como "Disparo em grupo" / "Convidados podem tocar" (não "editar"), e o changelog deixar claro que edição colaborativa vem depois. Resolver a expectativa, não só a dependência de código.
